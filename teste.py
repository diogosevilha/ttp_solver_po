...
# Instancias\eil51_n50_bounded-strongly-corr_01__.ttp

rota = [0, 26, 50, 45, 11, 46, 17, 3, 16, 36, 4, 37, 48, 8, 33, 29, 9, 38, 32, 44, 14, 43, 41, 39, 18, 40, 12, 24, 13, 23, 42, 6, 22, 5, 47, 7, 25, 30, 27, 2, 35, 34, 19, 21, 1, 28, 20, 49, 15, 10, 31]
itens_selecionados= [(1, 101, 1), (2, 202, 2), (3, 404, 4), (4, 202, 2), (10, 1868, 1468), (18, 222, 122), (19, 444, 244), (20, 888, 488), (21, 666, 366), (31, 1378, 1178), (49, 147, 47), (50, 294, 94)]

#Itens_selecionados = [1, 2, 3, 4, 10, 18, 19, 20, 21, 31, 49, 50]
Cities: 51 
Profit = 6816
Weight = 4016
Capacity = 4029
...
class FuncaoObjetivo:
    def __init__(self, capacidade_mochila, taxa_aluguel_por_tempo, velocidade_maxima, velocidade_minima, distancia_cidades):
        ...
        self.constante_velocidade = (velocidade_maxima - velocidade_minima) / capacidade_mochila

    def calcular_velocidade(self, peso):
        velocidade = self.velocidade_maxima - self.constante_velocidade * peso
        return max(velocidade, self.velocidade_minima)
    ...
    def calcular_lucro_com_itens(self, rota, itens_selecionados):
        ...
        for i in range(len(rota)):
            cidade_atual = rota[i]
            cidade_proxima = rota[(i + 1) % len(rota)]  # Volta ao início após a última cidade
...
def main():
    ...
    for arquivo_instancia in arquivos_instancias:
        ...
        valor_total_inicial, peso_total_inicial, tempo_total_inicial, aluguel_total_inicial, lucro_inicial = func_obj.calcular_lucro_com_itens(
            rota, itens_selecionados
        )

        print(valor_total_inicial)
        print(peso_total_inicial)
        print(tempo_total_inicial)
        print(aluguel_total_inicial)
        print(lucro_inicial)
        ...
        capacidade_mochila = 4029
        numero_de_cidades = 51 """

        # Lucro está batendo 6816 em ambos
        # Peso está batendo, 4016 em ambos;
        # tempo não está batendo, o do professor deu um tempo maior do que o meu, 573,568 contra 533,859
        # Qual o motivo para o tempo estar errado?
        ## diferente contagem?
        # 
